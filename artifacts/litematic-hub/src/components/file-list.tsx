...
import { useListFiles, useDeleteFile, getListFilesQueryKey, type LitematicFile } from "@workspace/api-client-react";
import { Card, CardContent, CardHeader, CardTitle, CardFooter } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Copy, Check, Download, Trash2, Eye, Box, FileJson, Layers, Shapes, AlertCircle } from "lucide-react";
...
export function FileList({ sessionId }: FileListProps) {
  ...
  const { data: files, isLoading, isError } = useListFiles(
    { sessionId },
    { query: { queryKey: getListFilesQueryKey({ sessionId }), enabled: !!sessionId } }
  );
  ...
  if (isError) {
    return (
      <div className="mt-8 p-6 text-center border rounded-lg bg-destructive/10 border-destructive/20 text-destructive flex flex-col items-center gap-2">
  ...
  if (!files || files.length === 0) {
    return (
      <div className="mt-8 p-12 text-center border border-dashed rounded-lg bg-card/30 text-muted-foreground flex flex-col items-center justify-center">
        ...
        <h3 className="text-lg font-medium text-foreground mb-1">{t.noFilesTitle}</h3>
        <p>{t.noFilesSub}</p>
      </div>
    );
  }
  ...
  return (
    <div className="mt-8 space-y-4">
      <h2 className="text-xl font-bold text-foreground">{t.yourFiles} ({files.length})</h2>
...
function FileCard({ file, sessionId }: { file: LitematicFile; sessionId: string }) {
  ...
  const deleteMutation = useDeleteFile({
    mutation: {
      onSuccess: () => {
        ...
        toast({ title: t.toastDeletedTitle, description: t.toastDeletedDesc });
      },
      onError: () => {
        toast({ variant: "destructive", title: t.toastDeleteFailTitle, description: t.toastDeleteFailDesc });
      },
    },
  });
  ...
  return (
    <Card className="flex flex-col bg-card border-border/40 hover:border-primary/40 transition-colors overflow-hidden">
      <CardHeader className="pb-3 border-b border-border/40 bg-secondary/20">
        <div className="flex justify-between items-start gap-2">
          ...
          <div className="flex -mr-2">
            <Button
              ...
              data-testid={`btn-download-${file.key}`}
            >
            ...
            <Button
              ...
              data-testid={`btn-delete-${file.key}`}
            >
        ...
        <div className="flex items-center text-xs text-muted-foreground gap-2 font-mono">
          ...
          <Button
            variant="ghost" size="icon"
            className="h-5 w-5 ml-auto hover:bg-transparent shrink-0"
            onClick={handleCopy}
            data-testid={`btn-copy-${file.key}`}
          >
      ...
      <CardContent className="pt-4 pb-2 flex-1 text-sm space-y-2 text-muted-foreground">
        <div className="flex items-center justify-between">
          <span className="flex items-center gap-1.5"><FileJson className="w-3.5 h-3.5 text-primary" /> {t.labelParts}</span>
          <span className="font-mono font-medium text-foreground">{file.partCount}</span>
        </div>
        <div className="flex items-center justify-between">
          <span className="flex items-center gap-1.5"><Layers className="w-3.5 h-3.5 text-primary" /> {t.labelSize}</span>
          <span className="font-mono">{Math.round(file.sizeBytes / 1024)} KB</span>
        </div>
        <div className="flex items-center justify-between">
          <span className="flex items-center gap-1.5"><Shapes className="w-3.5 h-3.5 text-primary" /> {t.labelBlocks}</span>
