...
import { ArrowLeft, Box, Copy, Check, Blocks, Database, LayoutGrid, MapMap, Info } from "lucide-react";
...
export default function FileDetail() {
  ...
  const { data: partData, isLoading: partLoading } = useGetFilePart(key, selectedPart || 0, {
    query: { queryKey: getGetFilePartQueryKey(key, selectedPart || 0), enabled: !!key && !!selectedPart }
  });
    ...
    return (
      <Layout>
        <div className="text-center py-20">
          <h2 className="text-2xl font-bold text-destructive mb-2">File not found</h2>
          <p className="text-muted-foreground mb-6">The file you are looking for does not exist or has been deleted.</p>
          <Link href="/">
  ...
  const handleCopyPart = () => {
    if (partData) {
      ...
      toast({ title: "Copied to clipboard", description: "Part JSON data copied." });
    }
  };
    ...
    <Layout>
      <div className="space-y-6 flex flex-col h-[calc(100vh-8rem)]">
        <div className="flex items-center justify-between shrink-0">
          ...
          <Button variant="outline" onClick={() => window.open(`/api/files/${file.key}/download`, '_blank')}>
            Download Original
          </Button>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 shrink-0">
          <StatCard icon={<Blocks />} label="Blocks" value={file.blockCount} />
          <StatCard icon={<Database />} label="Entities" value={file.entityCount} />
          <StatCard icon={<LayoutGrid />} label="Block Entities" value={file.blockEntityCount} />
          <StatCard icon={<Box />} label="Regions" value={file.regionCount} />
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 flex-1 min-h-0">
          <Card className="col-span-1 flex flex-col bg-card/50 overflow-hidden">
            <CardHeader className="py-3 px-4 bg-secondary/20 border-b shrink-0 flex flex-row items-center justify-between">
              <CardTitle className="text-sm font-medium">Parts ({file.partCount})</CardTitle>
            </CardHeader>
                ...
                <>
                  <CardTitle className="text-sm font-medium flex items-center gap-2">
                    ...
                    <span className="text-xs text-muted-foreground font-mono font-normal ml-2">
                      {partData?.data.length.toLocaleString() ?? 0} chars
                    </span>
                  </CardTitle>
            ...
            <div className="flex-1 relative min-h-0 bg-[#0d0d0d]">
              {!selectedPart ? (
                <div className="absolute inset-0 flex flex-col items-center justify-center text-muted-foreground/50">
                  ...
                  <p>Select a part from the list to view its JSON payload.</p>
                </div>
              ) : partLoading ? (
                <div className="absolute inset-0 p-4">
                  <Skeleton className="h-4 w-1/3 mb-2 bg-muted/20" />
                  <Skeleton className="h-4 w-1/2 mb-2 bg-muted/20" />
                  <Skeleton className="h-4 w-2/3 mb-2 bg-muted/20" />
                  <Skeleton className="h-4 w-1/4 mb-2 bg-muted/20" />
                </div>
              ) : partData ? (
  ...
  return (
    <Card className="bg-card/40 border-border/40">
      <CardContent className="p-4 flex items-center gap-4">
        <div className="p-2 bg-secondary/50 rounded-md text-primary shrink-0">
          {icon}
        </div>
        <div className="min-w-0">
          <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider">{label}</p>
          <p className="text-xl font-bold text-foreground font-mono truncate">{value !== undefined ? value.toLocaleString() : 'N/A'}</p>
        </div>
      </CardContent>
    </Card>
  );
