 ...
 */
/** 0 = no extra cap (only MAX_LITEMATIC_UPLOAD_MB). */
export const HOSTING_MAX_UPLOAD_MB = Math.max(
  0,
  parseInt(process.env.HOSTING_MAX_UPLOAD_MB ?? "0", 10) || 0,
);
...
export const HOSTING_OOM_HINT =
  "Не хватило памяти сервера при парсинге. Подождите минуту и попробуйте снова; " +
  "если повторяется — в RelaxDev добавьте env PARSE_HEAP_MB=1400 и полный редеплой.";
